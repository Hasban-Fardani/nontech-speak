...
import { translateTechnicalText, type AudienceType } from '../services/ai.service';
...
export const translationRoutes = new Elysia({ prefix: '/api/translation' })
  .post('/create', async ({ body, request, set }) => {
    ...
    try {
      // Call AI service
      const stream = await translateTechnicalText(technicalText, audienceType);
      ...
      if (!simplifiedText) {
        throw new Error("Translation failed: No content generated (likely API quota limit)");
      }
